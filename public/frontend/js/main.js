    ...
    var selectImages = function () {
        if ($(".image-select").length > 0) {
            ...
            selectIMG.find("option").each((idx, elem) => {
                ...
                let imgURL = selectOption.attr("data-thumbnail");
                if (imgURL) {
                    // Ensure correct path for asset images
                    if (!imgURL.startsWith('http') && !imgURL.startsWith('/')) {
                        imgURL = window.location.pathname.includes('/redragon/') 
                            ? `/redragon/${imgURL}` 
                            : `/${imgURL}`;
                    }
                    selectOption.attr(
  ...
  ------------------------------------------------------------------------------------- */
    var totalPriceVariant = function () {
        $(".tf-product-info-list,.tf-cart-item").each(function () {
            ...
            // ⚠️ REMOVED: btn-increase and btn-decrease listeners
            // These are now handled by setupQuantityHandlers() in product_single.blade.php
            // to avoid duplicate event triggers
  ...
  ------------------------------------------------------------------------------------- */
    ...
    var scrollBottomSticky = function () {
        $(window).on("scroll", function () {
            ...
            var productElement = $(".tf-product-info-by-btn");
            ...
            var offsetData = productElement.offset();
            if (!offsetData) {
                return; // Exit if offset is undefined
            }

            var height = offsetData.top + 50;
            // console.log(height);
...
document.addEventListener('click', function(event) {
    ...
    var navItem = event.target.closest('.nav-icon-item');
    if (accountMenu && !navItem) {
...
//     var menuLinks = document.querySelectorAll('#mobileMenu a[href]:not([href="#"])');
...
//     });
// });
// Offcanvas configuration helper
document.addEventListener('DOMContentLoaded', function() {
    var mobileMenu = document.getElementById('mobileMenu');
    if (mobileMenu && typeof bootstrap !== 'undefined') {
        // Ensure offcanvas is properly configured
        new bootstrap.Offcanvas(mobileMenu, {
            backdrop: true,
            scroll: true,
            keyboard: true
        });
